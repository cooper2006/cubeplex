...
class WeChatGateway:
    ...
    DEFAULT_QRCODE_POLL_TIMEOUT = 600.0
    ...
    def is_open(self) -> bool:
        return bool(not self._stopping and (self._bot_token or self._qrcode_login_enabled))

    @property
    def is_authenticated(self) -> bool:
        """True once an iLink bot token exists (QR confirmed or token stored)."""
        return bool(self._bot_token)

    def last_qrcode_url(self) -> str | None:
        """Return the QR URL the user is looking at, without refreshing it.

        Used once the QR is already confirmed: minting a fresh one would
        orphan the scan the gateway just authenticated with.
        """
        url = str(self._auth_state.get("qrcode_img_content") or "").strip()
        if url:
            return url
        qrcode = str(self._auth_state.get("qrcode") or "").strip()
        return self._make_ilink_qr_url(qrcode) if qrcode else None
    ...
    async def force_get_qrcode_url(self) -> str | None:
        ...
        url = await self._request_qrcode()
        if url:
            # A new QR is only useful if something polls it — without this the
            # gateway keeps waiting on the code the user already abandoned.
            self._wake_qr_auth()
        return url
    ...
    async def _check_binding_code(self, code: str, chat_id: str, context_token: str) -> bool:
        ...
        # Sync the binding state's QR into the gateway so polling continues
        # against the right QR even if the user refreshed the frontend.
        try:
            import json as _json

            _payload = _json.loads(raw)
            _qrcode = str(_payload.get("qrcode") or "").strip()
            if _qrcode:
                stored = str(self._auth_state.get("qrcode") or "").strip()
                if stored and stored != _qrcode:
                    _img = str(_payload.get("qrcode_img_content") or "").strip() or None
                    self._save_auth_state(
                        status="pending", qrcode=_qrcode, qrcode_img_content=_img or None
                    )
                    self._wake_qr_auth()
                    logger.info(
                        "[WeChat] synced binding QR {} into gateway (was {})",
                        _qrcode[:12],
                        stored[:12],
                    )
        except Exception:
        ...
        if not self._bot_token:
            await self._send_connection_reply(
                chat_id, context_token, "请等待二维码绑定完成后再发送连接码。"
            )
            return True
    ...
    async def _update_account_token(self, bot_token: str, chat_id: str, context_token: str) -> None:
        ...
        try:
            async with self._session_maker() as session:
                ...
                # Read existing credential to preserve other fields (e.g. qrcode_login_enabled)
                existing = await svc.get_decrypted(
                    credential_id=self._account.credential_id, requesting_kind="im_bot"
                )
                ...
                if existing:
                    try:
                        ...
                        pass
                existing_data["bot_token"] = bot_token
                existing_data["qrcode_login_enabled"] = True
                # bot_open_id is the signal the runtime list reads to leave the
                # "never_connected" state — without it a bound WeChat account
                # still renders as 未连接.
                existing_data["bot_open_id"] = self._ilink_bot_id or chat_id
                await svc.update(
                    credential_id=self._account.credential_id,
                    plaintext=json.dumps(existing_data),
                )
                ...
                await session.execute(
                    ...
                    .where(IMConnectorAccount.id == self._account.id)
    ...
    async def _handle_update(self, raw_message: Any) -> None:
        ...
        chat_id = str(
            raw_message.get("from_user_id") or raw_message.get("ilink_user_id") or ""
        ).strip()
        if not chat_id:
            return
        ...
        if context_token:
            ...
            thread_ts = (
                str(raw_message.get("client_id") or raw_message.get("msg_id") or "").strip() or None
            )
            if thread_ts:
    ...
    async def send_text(self, chat_id: str, context_token: str, text: str) -> str | None:
        """Send text to a WeChat chat. Returns the message id, or None on failure.

        iLink does not hand back a server-side message id, so we return the
        ``client_id`` we generated — enough for the tailer to tell "sent"
        from "failed".
        """
        if not self._bot_token:
            return None
        ...
        if data.get("ret") not in (0, None):
            logger.warning("[WeChat] sendmessage ret=%s", data.get("ret"))
            return None
        return client_id
